            ...
            try {
                ...
                if (error.response) {
                    ...
                    if (status === 400) {
                        ...
                        const newErrors = {};
                        if (data.name) newErrors.name = Array.isArray(data.name) ? data.name[0] : data.name;
                        if (data.email) newErrors.email = Array.isArray(data.email) ? data.email[0] : data.email;
                        if (data.subject) newErrors.subject = Array.isArray(data.subject) ? data.subject[0] : data.subject;
                        if (data.message) newErrors.message = Array.isArray(data.message) ? data.message[0] : data.message;
                        
                        if (Object.keys(newErrors).length > 0) {
                            setErrors(newErrors);
                        } else {
                            dispatch(showNotification({
                                message: 'Error en los datos del formulario. Por favor, revisa los campos.',
                                type: 'error'
                            }));
                        }
                    } else {
