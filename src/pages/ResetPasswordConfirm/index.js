...
const ResetPasswordConfirm = () => {
    ...
    // No validar el token automáticamente - permitir que el usuario trate de usarlo
    useEffect(() => {
        // Solo validar que el token existe en la URL
        if (!token) {
            setTokenValid(false);
            setBackendError('No se proporcionó token de restablecimiento en la URL');
        }
    }, [token]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (validateForm()) {
            ...
            try {
                ...
                const response = await authApi.resetPassword(token, formData.password, formData.password2);
                ...
                if (error.response) {
                    ...
                    if (status === 400) {
                        // Error de validación o token inválido
                        if (data.token) {
                            setTokenValid(false);
                            setBackendError('El token de restablecimiento es inválido o ha expirado');
                        } else if (data.password) {
                            setErrors({ password: Array.isArray(data.password) ? data.password[0] : data.password });
                        } else if (data.password2) {
                            setErrors({ password2: Array.isArray(data.password2) ? data.password2[0] : data.password2 });
                        ...
                        setTokenValid(false);
                        setBackendError('El token de restablecimiento no fue encontrado');
                    } else {
        ...
        return (
            <ResetContainer>
                ...
                <TokenError>
                    ...
                    <p>{backendError || 'El enlace para restablecer la contraseña no es válido o ha expirado. Por favor, solicita un nuevo enlace.'}</p>
